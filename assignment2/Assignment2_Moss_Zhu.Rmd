...
=======
ggplot(charleston_sav_analysis, 
       aes(x = as.factor(Year), y = `Total Operating Revenue`/1000000, 
           color = `Airport Name`, 
           group = `Airport Name`)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  theme_minimal() +
  scale_y_continuous(labels = scales::dollar_format()) +
  labs(title = "Total Operating Revenue (2021-2023)",
       subtitle = "Charleston Int'l vs Savannah Int'l",
       y = "Revenue (Millions USD)",
       x = "Year",
       color = "Airport Name") +
  theme(legend.position = "bottom") +
...
summary_table <- revenue_changes %>%
 select(`Airport Name`, Year,
        `Total Operating Revenue`,
        Total_Revenue_YoY,
        Aero_Revenue_YoY,
        Non_Aero_Revenue_YoY) %>%
 arrange(`Airport Name`, Year) %>%
 mutate(
   Year_Change = paste(Year-1, "to", Year),
   # Rename and format percentage columns
   `Total Revenue Change` = sprintf("%.1f%%", Total_Revenue_YoY),
   `Aeronautical Revenue Change` = sprintf("%.1f%%", Aero_Revenue_YoY),
   `Non-Aeronautical Revenue Change` = sprintf("%.1f%%", Non_Aero_Revenue_YoY)
 ) %>%
 filter(!is.na(Total_Revenue_YoY)) %>%  # Remove rows where YoY can't be calculated
 select(`Airport Name`, Year_Change, 
        `Total Revenue Change`,
        `Aeronautical Revenue Change`,
        `Non-Aeronautical Revenue Change`) %>%
 arrange(Year_Change)

# Print formatted table
print("Revenue Changes by Year")
print(summary_table)

# Improved revenue composition plot
ggplot(charleston_sav_analysis, 
      aes(x = as.factor(Year))) +
 geom_line(aes(y = `Total Aeronautical Revenue`/1000000, color = "Aeronautical", group = 1), size = 1.2) +
 geom_line(aes(y = `Total Non-Aeronautical Revenue`/1000000, color = "Non-Aeronautical", group = 1), size = 1.2) +
 geom_point(aes(y = `Total Aeronautical Revenue`/1000000, color = "Aeronautical"), size = 3) +
 geom_point(aes(y = `Total Non-Aeronautical Revenue`/1000000, color = "Non-Aeronautical"), size = 3) +
 facet_wrap(~`Airport Name`) +
 theme_minimal() +
 scale_y_continuous(labels = scales::dollar_format()) +
 scale_color_manual(values = c("#FF7F7F", "#7FB3D5")) +
 labs(title = "Revenue Composition (2021-2023)",
      subtitle = "Aeronautical vs Non-Aeronautical Revenue",
      y = "Revenue (Millions USD)",
      x = "Year",
      color = "Revenue Type") +
 theme(
...
```{r}
# First calculate the year-over-year changes for each category
